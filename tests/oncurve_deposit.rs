...
/// signer privilege a real smart-wallet program would grant its PDA through
...
#[test]
fn on_curve_deposit_address_controlled_by_an_off_curve_smart_wallet() {
    // Signature verification is disabled only so an off-curve PDA can stand in
    // for the signer privilege that a production smart-wallet program would
    ...
    // P: the off-curve smart-wallet authority.
    ...
    // D: an on-curve, Token-Program-owned create_with_seed address based on P.
    let (deposit_seed, deposit_address) = grind_on_curve_deposit_address(&smart_wallet);
    assert!(deposit_address.is_on_curve());
    assert_eq!(
        deposit_address,
        Address::create_with_seed(&smart_wallet, &deposit_seed, &TOKEN_ID).unwrap()
    );

    // Create D without a D signature. Required signers are only the payer and
    // the base P. Then initialize D as a 1-of-1 SPL Token multisig whose member
    // is P.
    ...
    let initialize_deposit_multisig =
        initialize_multisig2(&TOKEN_ID, &deposit_address, &[&smart_wallet], 1).unwrap();
    ...
    assert_eq!(multisig.signers[0], smart_wallet);
    ...
    // the on-curve deposit address, and a receiver.
    ...
    // Withdraw: D is the token authority/multisig account, while P is its one
    // configured signer. In production, this exact signer privilege comes from
    // the smart-wallet program's invoke_signed CPI.
    ...
    .unwrap();
    send_with_phantom_signer(&mut svm, &payer, &[withdraw_tokens], &smart_wallet);
    ...
    // validates D with member P, and leaves D's rent reserve behind.
    let receiver_before = svm.get_balance(&receiver).unwrap_or(0);
    ...
    .unwrap();
    send_with_phantom_signer(&mut svm, &payer, &[withdraw_sol], &smart_wallet);

    assert_eq!(svm.get_balance(&deposit_address).unwrap(), multisig_rent);
    assert_eq!(
        svm.get_balance(&receiver).unwrap(),
